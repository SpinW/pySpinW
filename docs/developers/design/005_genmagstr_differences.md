Differences between python `genmagstr` and MATLAB
===============================================

We attempt to recreate `genmagstr` as close as possible to the MATLAB version. 
However the different structure of pyspinw means that there will be some non-trivial differences.

Below are the MATLAB docs for `genmagstr`, along with notes on the differences...
...
Mode that determines how the magnetic structure is generated:
  ...
  * `'func'` (reads `func`, `x0`)
    ...
    <span style="color:red">**This mode is completely removed**: the reasoning is 
     that people that want more control should just use the python interface </span> 
...
: <span style="color:red">**Details of the implementation are not the same**: 
  the way that commensurate and incommensurate vectors are represented is different,
  and the tolerances are not the same. It should work roughly the same way though.
  </span>
...
`'epsilon'`
: The smallest value of incommensurability that is
  tolerated without warning in lattice units. Default is $10^{-5}$.

: <span style="color:red">**Commensurate and incommensurate vectors 
  now handled differently** - tolerances are now 'global' variables in
  the tolerances package </span>
