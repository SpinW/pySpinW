...
def test_heisenberg_ferromagnet(direction):
    ...
    couplings = generate_exchanges(sites, UnitCell(1, 1, 1), max_distance=1,
...
def test_heisenberg_antiferromagnet_supercell():
    ...
    couplings = generate_exchanges(sites, UnitCell(1, 1, 1), max_distance=0.6,
...
def test_heisenberg_antiferromagnet_single_cell():
    ...
    couplings = generate_exchanges(sites, UnitCell(1, 1, 1), max_distance=1.1,
                                   direction_filter=BiDirectionFilter([1, 0, 0]), j=1)
...
def test_hexagonal_cell():
    ...
    couplings = generate_exchanges(sites, UnitCell(1, 1, 2, gamma=120), max_distance=1.1, j=1)
...
def test_cubic_cell():
    ...
    couplings = generate_exchanges(sites, UnitCell(1, 1, 1), bond=1, j=1)
    assert len(couplings) == 3

    # Next-nearest neighbours (1 each in +x+y, +x-y, +x+z, +x-z, +y+z, +y-z directions)
    couplings = generate_exchanges(sites, UnitCell(1, 1, 1), bond=2, j=1)
    assert len(couplings) == 6

    # 3rd-nearest neighbours (1 in each x+y+z, x-y+z, x+y-z, x-y-z directions)
    couplings = generate_exchanges(sites, UnitCell(1, 1, 1), bond=3, j=1)
